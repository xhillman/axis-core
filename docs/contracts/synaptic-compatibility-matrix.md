...
|---|---|---|---|
| `0.12.1` | `0.3.x` (`>=0.3.0,<0.4.0`) | verified | `tests/adapters/memory/test_synaptic.py`, `tests/adapters/memory/test_synaptic_integration.py`, `tests/engine/test_registry.py`, local interop run on `0.3.0` |
| `0.12.1` | `<0.3.0` | not_supported | Adapter init version gate fails fast with actionable upgrade guidance |
| `0.12.1` | `>=0.4.0` | not_verified | Outside currently validated range; blocked by adapter init version gate |
